...
inline void HypoelastoplasticModel::EvalCoeffs() const
{
   mu = c_mu->Eval(*T, T->GetIntPoint());
   K = c_K->Eval(*T, T->GetIntPoint());
   if (c_g)
...
double HypoelastoplasticModel::EvalW(const DenseMatrix &J) const
{
   int dim = J.Width();
...
void HypoelastoplasticModel::EvalP(const DenseMatrix &J, DenseMatrix &P) const
{
   int dim = J.Width();
...
void HypoelastoplasticModel::AssembleH(const DenseMatrix &J, const DenseMatrix &DS,
                                const double weight, DenseMatrix &A) const
{
   int dof = DS.Height(), dim = DS.Width();
